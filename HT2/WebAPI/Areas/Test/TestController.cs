using DAL.DbContexts;
using Domain.Models;
using Microsoft.AspNetCore.Mvc;
using System;
using System.Collections.Generic;
using System.Linq;

namespace WebAPI.Areas.Test
{
	[Route("test")]
	[ApiController]
	public class TestController: ControllerBase
	{
		private readonly DbContextBase dbContext;

		public TestController(DbContextBase dbContext)
		{
			this.dbContext = dbContext;
		}

		[HttpGet]
		public List<Book> FillDb()
		{
			const int bookCount = 100;
			const int reviewsPerBook = 50;
			const int ratingPerBook = 50;

			var db = new DbContextBase();

			for (int i = 1; i <= bookCount; i++)
			{
				db.Books.Add(new()
				{
					Title = "Title" + i,
					Cover = "data:image/jpeg;base64,",
					Content = "123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 123456789 ",
					Author = "Author" + i,
					Genre = "Genre" + i,
				});
			}
			db.SaveChanges();

			var random = new Random();
			for (int i = 1; i <= bookCount; i++)
			{
				for (int j = 1; j <= reviewsPerBook; j++)
				{
					db.Reviews.Add(new()
					{
						Message = "Message" + (j + i * 10),
						BookId = i,
						Reviewer = "Reviewer" + (j + i * 10),
					});
				}

				for (int j = 1; j <= ratingPerBook; j++)
				{
					db.Ratings.Add(new()
					{
						BookId = i,
						Score = random.Next(40) / 2
					});
				}
			}

			db.SaveChanges();

			return db.Books.ToList();
		}
	}
}
